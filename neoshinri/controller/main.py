...
def generate_tasks(user_input: str):
    ...
    return Task(task_id=str(uuid.uuid4()) ,task_type= MessageType.COMMAND ,description="Generate a file based on user input: " + user_input)
...
def send_to_tunnel(acp_message: ACPMessage):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            raise HTTPException(status_code=response.status_code, detail="Error from Tunnel: " + response.text)
